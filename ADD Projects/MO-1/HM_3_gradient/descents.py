...
class VanillaGradientDescent(BaseDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        vanilla_gradient = -self.lr() * gradient
        self.w = self.w + vanilla_gradient
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        batch_indexes = np.random.randint(low=0, high=len(y), size=self.batch_size)
        batch = x[batch_indexes, :]
        prediction = y[batch_indexes]
        #gradient = 2 / self.batch_size * (np.dot(np.transpose(batch), np.dot(batch, (self.w - prediction))))
        gradient = 2 / self.batch_size * (np.transpose(batch) @ (batch @ self.w - prediction))
        return gradient
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        difference = -self.lr() * m_head / (np.sqrt(v_head) + self.eps)
        self.w = self.w + difference
        return  difference
